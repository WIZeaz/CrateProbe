...
def test_detect_systemd_available():
    ...
    limiter = ResourceLimiter(
        use_systemd=True, max_memory_gb=20, max_runtime_seconds=86400  # 24 hours
    )
...
def test_build_resource_command():
    ...
    limiter = ResourceLimiter(
        use_systemd=False, max_memory_gb=20, max_runtime_seconds=86400  # 24 hours
    )
...
def test_apply_resource_limits():
    ...
    limiter = ResourceLimiter(
        use_systemd=False, max_memory_gb=1, max_runtime_seconds=3600  # 1 hour
    )
